...
class SEFModelUpgrade extends JModel
{
    function getUpgradeExts()
    {
        if (!isset($this->_upgradeExts)) {
            ...
            // Sort the extensions
            uasort($extensions, array($this, '_cmpExts'));
        ...
        if (!isset($this->_regInfo) ) {
            ...
            if( trim($sefConfig->artioDownloadId) != '' ) {
                ...
                if( ($response === false) || ($response->code != 200) ) {
                    ...
                    if (in_array($regInfo->code, array(10, 20, 30, 40))) {
                        // Download id found
                        if (count($lines) < 3) {
                            // Wrong response
                            JError::raiseNotice(100, JText::_('ERROR_REG_CHECK_FAIL'));
						...
						if ($regInfo->code == 30) {
                         ...
                         // Is upgrade inactive
						if ($regInfo->code == 40) {
                            JError::raiseNotice(100, JText::_('ERROR_DOMAIN_DOES_NOT_MATCH') . ' ' . JText::_('INFO_YOU_GET_FREE_VERSION'));
                        }                 
                    }
                ...
                $link2 = '</a>';
                $enterIdText = sprintf(JText::_('INFO_ENTER_DOWNLOAD_ID'), $link1, $link2);
                JError::raiseNotice(100, JText::_('Download ID not set.') . ' ' . JText::_('INFO_YOU_GET_FREE_VERSION') . ' ' . $enterIdText);
                return null;
            }
