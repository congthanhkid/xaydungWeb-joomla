...
class SEFTools
{
    ...
    function GetSEFGlobalMeta() {
        return '03bfec0d5a9fc3742b80aa5388bc1d8a'; // sef.global.meta
    }
    ...
    function arraysEqual(&$arr1, &$arr2) {
        ...
        foreach ($arr1 as $key => $val) {
            if (!isset($arr2[$key]) || ($arr2[$key] != $val)) {
                return false;
            }
        }
    ...
    function getAllSubdomains()
    {
        ...
        if (!isset($subdomains)) {
            ...
            $db->setQuery($query);
            $subdomains = $db->loadObjectList();
        }

        return $subdomains;
    }
    
	function getSubDomain($uri,&$titlepage) {
        $titlepage=false;
		...
		for($i=0;$i<count($subdomains);$i++) {
			$Itemids=explode(",",$subdomains[$i]->Itemid);
			if($Itemid==$subdomains[$i]->Itemid_titlepage) {
				// Check whole menu query
                $menu =& JSite::getMenu(true);
                $item = $menu->getItem($Itemid);
                if ($item != null) {
                    // Check queries except Itemids
                    $query1 = $uri->getQuery(true);
                    $query2 = $item->query;
                    if (isset($query1['Itemid'])) {
                        unset($query1['Itemid']);
                    }
                    if (isset($query2['Itemid'])) {
                        unset($query2['Itemid']);
                    }
                    
                    if (SEFTools::arraysEqual($query1, $query2)) {
                        $titlepage = true;
                    }
                }
                
				return $subdomains[$i]->subdomain;
			}
			if($option==$subdomains[$i]->option) {
				return $subdomains[$i]->subdomain;	
			}
			if(strlen($subdomains[$i]->Itemid) && in_array($Itemid,$Itemids)) {
				return $subdomains[$i]->subdomain;
			}
		}
		return null;
