...
// RELATED PRODUCTS MODIFIED: IF REL PRODS DEFINED, WILL USE, ELSE RANDOM (3) PRODS DISPLAYED
...
// Let's have a look whether the product has related products.
$q = "SELECT product_sku, related_products FROM #__{vm}_product,#__{vm}_product_relations ";
$q .= "WHERE #__{vm}_product_relations.product_id='$product_id' AND product_publish='Y' ";
$q .= "AND FIND_IN_SET(#__{vm}_product.product_id, REPLACE(related_products, '|', ',' )) LIMIT 0, 50";
$db->query( $q );
} else	{

//&nbsp; If no related items are selected for this product, this shows randomly selected products from the same category of the displayed product
