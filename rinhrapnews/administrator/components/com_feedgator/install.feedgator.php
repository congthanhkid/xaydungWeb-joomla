...
$query = 'SHOW TABLES LIKE '.$db->Quote('#__feedgator');
$db->setQuery($query);
if($db->loadResult()) {
	$query = 'SHOW COLUMNS FROM '.$db->nameQuote('#__feedgator');
	$db->setQuery($query);
	if($rows = $db->loadAssocList()) {

		// 2.0.x to 2.3RC4
		if(!FeedgatorUtility::in_array_recursive('filtering',$rows)) {
			$query = 	"ALTER TABLE ".$db->nameQuote('#__feedgator') .
		...
		elseif(!FeedgatorUtility::in_array_recursive('content_type',$rows)) {
			...
			$query =	"SELECT * FROM ".$db->nameQuote('#__feedgator');
			$db->setQuery($query);
			...
			//	if(substr($erow->params,-1,2) != "\n") $erow->params .= "\n";
				$erow->params = implode("\n", $txt)."\n".$erow->params;
				$db->updateObject( '#__feedgator', $erow, 'id' );
			}
			unset($erows,$erow);
		...
		elseif(FeedgatorUtility::in_array_recursive('content_type',$rows)) {
			...
			$query = 'SHOW COLUMNS FROM '.$db->nameQuote('#__feedgator_imports');
			$db->setQuery($query);
			if(!$rows = $db->loadAssocList()) {
				$from = '2.3b1-4'; //dev
				$upgrade = 1;
			} else {
				$query = 	"ALTER TABLE ".$db->nameQuote('#__feedgator_imports') .
