<?php
/**
 * @version		1.6.0.71 helpers/importer.php
...
class j2xmlImporter
{
	...
	static function import($xml, $filever, $xmlrpc = false)
	{
		...
		$keep_userids = $params->get('keep_userids', '0');
		...
		if ($import_users)
		{
			foreach($xml->xpath("user") as $record)
			{
				...
				$query = 'SELECT id, name'
					. ' FROM #__users'
					. ' WHERE'. ((($attributes->mapkeystotext == 'false') || ($keep_userids == 1))
						? ' id = '.$id
				...
				if (!$user || ($import_users == 2))
				{
					...
					if ($table->save($data))
					{
						if (!$user && (($attributes->mapkeystotext == 'false') || ($keep_userids == 1)))
						{
