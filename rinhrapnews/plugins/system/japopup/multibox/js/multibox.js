...
var multiBox = new Class({
	...
	showContent: function(){
		...
		if(this.type == 'image'){
			...
			if(this.contentObj.url.test(/youtube/)){
				this.contentObj.url = this.contentObj.url.replace('/v/', '/embed/');
			}
			new Element('iframe', {
		...
		if(this.options.descClassName){
			...
			if(ignoreDesc != true){
				...
				if(this.contentToLoad.desc && this.description){
	...
	hideContent: function(){
		this.box.addClass('MultiBoxLoading');
		try{
			if($('MultiBoxContentContainer').getElements('iframe').length){
				$('MultiBoxContentContainer').getElements('iframe').each(function(ifm){
					ifm.src = 'about:blank';
				});
			}
		}catch(e){
		...
		this.removeContent.bind(this).delay(600);
	},
	...
	removeContent: function(){
		...
		if($('MultiBoxContentContainer')){
			$('MultiBoxContentContainer').getElements('iframe').destroy();//dispose() instead of destroy() as IE 6&7 crashes			
