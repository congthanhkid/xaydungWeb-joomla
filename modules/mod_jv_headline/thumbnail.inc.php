...
if(!class_exists('JVThumbnail')){
	class JVThumbnail {
		...
		function resize_image($width,$height,$crop=true,$aspect=true){
			...
			if(function_exists("ImageCreateTrueColor")) {
				$this->workingImage = ImageCreateTrueColor($dst->w,$dst->h);
			}
			else {
				$this->workingImage = ImageCreate($dst->w,$dst->h);
			}
			if(function_exists("ImageCreateTrueColor")) {
				$this->workingImage = ImageCreateTrueColor($dst->w,$dst->h);
			}
			...
			$this->oldImage = $this->workingImage;
			$this->newImage = $this->workingImage;
			$this->currentDimensions['width'] = $dst->w;
			$this->currentDimensions['height'] = $dst->h;
		}
		//End resize image
