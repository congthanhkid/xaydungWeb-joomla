...
class FeedgatorController extends JController
{
	...
	*/
	...
	private function importAll($update=false)
	{
		$ajax = JRequest::getInt('ajax',0,'get');
		if($ajax) {
			$this->_db->setQuery( 'SELECT id, title FROM #__feedgator WHERE published = 1 ORDER BY id' );
			$formData = $this->_db->loadAssocList();
			echo json_encode($formData);
			jexit();
		} else {
			$this->_db->setQuery( 'SELECT id FROM #__feedgator WHERE published = 1 ORDER BY id' );
