...
if(ml!=0)li.childcontent.setStyle('margin-left',ml);if(mtop=li.getElement('.childcontent-inner')){var topleft=lcor.left-mtop.getCoordinates().left+lcor.width/2-4.5;mtop.setStyle('background-position',topleft+"px top");}}else{var lcor=li.getCoordinates();var ccor=li.childcontent.getCoordinates();if(!ccor.width)
{li.childcontent.setStyle('display','block');ccor=li.childcontent.getCoordinates();li.childcontent.setStyle('display','none');}
var mr=0;var r=lcor.right;var l=r-ccor.width;if(this.wrapper){var wcor=this.wrapper.getCoordinates();l=l-wcor.left+10;r=wcor.right-r+10;}else{r=window.getWidth()-r+10;}
if(r<0||l+r<0){mr=-r;}else if(l<0){mr=l;}
if(mr!=0)li.childcontent.setStyle('margin-right',mr);if(mtop=li.getElement('.childcontent-inner')){var topleft=lcor.left-mtop.getCoordinates().left+lcor.width/2-4.5;mtop.setStyle('background-position',topleft+"px top");}}}else{var lcor=li.getCoordinates();var ccor=li.childcontent.getCoordinates();if(!ccor.width)
{li.childcontent.setStyle('display','block');ccor=li.childcontent.getCoordinates();li.childcontent.setStyle('display','none');}
...
if(r<0){li.childcontent.setStyle('margin-left',-ccor.width+20);li.childcontent_inner.setStyle('margin-left',li._w+20);if(mtop=li.getElement('.childcontent-inner')){var topleft=lcor.left-mtop.getCoordinates().left+lcor.width/2-4.5;mtop.setStyle('background-position',topleft+"px top");}}}}});

/* d3.default.js */

function createFloating(dir){var body=document.body||document.getElementsByTagName('body')[0],newpar=document.createElement('div');newpar.setAttribute("id","floater_"+dir);newpar.innerHTML=document.getElementById("floater_content_"+dir).innerHTML;body.insertBefore(newpar,body.childNodes[0]);}
function moveFollowleft(topX,topY){var heightwindows=$(window).getSize().y;var target=$("floater_left");var max=window.getScrollSize().y-target.getSize().y-75;var scrollPos=document.documentElement.scrollTop;if(scrollPos==0){scrollPos=document.body.scrollTop;}
var y=Math.min(scrollPos+((heightwindows-target.getSize().y)/2),max);if((y<topY)||(scrollPos==0))y=topY;if(typeof fx_left!="undefined")fx_left.stop();fx_left=new Fx.Morph(target,{duration:1000,wait:false,link:'chain'});fx_left.options.transition=Fx.Transitions.Back.easeInOut;fx_left.start({'top':[y],'left':topX});}
function moveFollowright(topX,topY){var heightwindows=$(window).getSize().y;var target=$("floater_right");var max=window.getScrollSize().y-target.getSize().y-75;var scrollPos=document.documentElement.scrollTop;if(scrollPos==0){scrollPos=document.body.scrollTop;}
var y=Math.min(scrollPos+((heightwindows-target.getSize().y)/2),max);if((y<topY)||(scrollPos==0))y=topY;if(typeof fx_right!="undefined")fx_right.stop();fx_right=new Fx.Morph(target,{duration:1000,wait:false,link:'chain'});fx_right.options.transition=Fx.Transitions.Back.easeInOut;fx_right.start({'top':[y],'right':topX});}
...
requesting=true;jausersettingajax=new Request.HTML({url:link,method:'get',update:document.id(parent),data:form.toQueryString(),onSuccess:function(data){submit_bt.disabled=false;requesting=false;document.getElements('.jahasTip').each(function(el){var title=el.get('title');if(title){var parts=title.split('::',2);el.store('tip:title',parts[0]);el.store('tip:text',parts[1]);}});var JTooltips=new Tips(document.getElements('.jahasTip'),{maxTitleChars:50,fixed:false,className:'tool-tip janews-tool'});}.bind(this)}).send()}.bind(this))}.bind(this))}},reloadJS:function(parent){parent.getElements('script').each(function(script){if(script.src){new Element('script',{'type':'text/javascript','src':script.src}).inject($(document.body).getElement('head'));}else{eval(script.innerHTML);}});},showElement:function(obj,height){if(!obj.fx){obj.fx=new Fx.Tween(obj);}
...
var H=Math.max($('ja-zinfp-'+moduleid).getElement('.ja-zinfp-normal-wrap').offsetHeight+13,(this.panelwrap.offsetHeight+$('ja-zinfp-'+moduleid).getElement('.ja-zinfp-links-actions').offsetHeight));this.panelwrap.getParent().setStyle('height',H);this.next_bt.addEvent('click',function(){this.timer=$clear(this.timer);this.next();if(this.options.autoplay==1){this.timer=this.next.periodical(this.options.autoplay_duration,this);}}.bind(this));this.prev_bt.addEvent('click',function(){this.timer=$clear(this.timer);this.previous();if(this.options.autoplay==1){this.timer=this.next.periodical(this.options.autoplay_duration,this);}}.bind(this));}},next:function(){if(!this.mainfx)this.mainfx=new Fx.Tween(this.panelwrap,{duration:this.options.duration});var moduleid=this.options.moduleid;this.current_activePanel=this.panelwrap.getElement('div.active');if(this.current_activePanel.getNext()==null){this.activePanel=this.panelwrap.getFirst();this.counter.set('text',1);}
