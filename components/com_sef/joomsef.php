...
class JoomSEF
{
        ...
        if (SEFTools::JoomFishInstalled()) {
            ...
            if ($sefConfig->langPlacement == _COM_SEF_LANG_DOMAIN) {
                ...
                if (!empty($lang)) {
                	...
                	// Lang must be saved because of domains with same end title but different path as different category
                	// names
                	foreach($sefConfig->jfSubDomains as $sub_lang=>$subdomain) {
                		if($subdomain==$curdomain) {
                			$uri->setVar('lang',$sub_lang);
                		}
                	}
                }
    ...
    function parse(&$uri)
    {
        ...
        // Fix for missing language when Jfrouter is enabled
        if (SEFTools::JoomFishInstalled() && ($sefConfig->langPlacement == _COM_SEF_LANG_PATH) && JPluginHelper::isEnabled('system', 'jfrouter')) {
            $lng = $uri->getVar('lang');
            if (!empty($lng)) {
                if (substr($path, 0, strlen($lng)) != $lng) {
                    $path = $lng.'/'.ltrim($path, '/');
                }
            }
        }
        ...
        if ($sefConfig->fixIndexPhp && (substr($path, 0, 10) == 'index.php/') && (count($_POST) == 0)) {
            // Make sure we only remove the index.php/, not the query or fragment parts
            $newPath = substr($path, 10);
            if ($newPath[0] != '/') {
                $newPath = '/' . $newPath;
            }
            $uri->setPath($newPath);
            
            $mainframe->redirect($uri->toString(), '', 'message', true);
            exit();
        }
                ...
                ($sefConfig->mainLanguage != '0'))
            {
                ...
                if ($shouldRedir) {
                    if ((!empty($lang) && ($sc != $lang)) || (empty($lang) && ($sc != $sefConfig->mainLanguage))) {
                        ...
                        $mainframe->redirect($href, '', 'message', true);
                        exit();
                    }
                }
            }
            ...
            if (!empty($code)) {
                ...
                if( $resetLang || !$jfrouterEnabled ) {
                    ...
                    if ((!$jfrouterEnabled && $sefConfig->jfLangCookie) || ($jfrouterEnabled && $jfrparams->get('enableCookie', 1))) {
                        if (!headers_sent()) {
                            // Can't set cookie if headers already sent
    ...
    function _sendHeader($header)
    {
        ...
        // 25.4.2012 dajo: don't die on headers sent
        //else {
     ...
     * @param array sitemapParams Sitemap parameters for the URL (indexed, changefreq, priority) 
     * @return array
     */
    ...
    {
        // Compatibility fix for extensions without host support (probably only VM version 2.0.38)
        if (is_array($host) && is_null($sitemapParams)) {
            $sitemapParams = $host;
            $host = null;
        }
    ...
    function _saveNewURL($Itemid, &$metadata, $priority, $temploc, $url, $check404 = false, $host = null, $sitemapParams = null)
    {
        ...
        "VALUES (".$db->Quote($temploc).", ".$db->Quote(html_entity_decode(urldecode($url))).", ".$db->Quote($priority). $val . $metavals . ", " . $traceinfo . ", '{$sm_indexed}', '{$sm_date}', '{$sm_frequency}', '{$sm_priority}', '{$autolock}', '{$host}')";
        $db->setQuery($query);
